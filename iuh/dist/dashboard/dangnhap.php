...
session_start();
error_reporting(0);
if (isset($_SESSION['admin']) || isset($_SESSION['giangvien'])) {
  ...
  exit;
} elseif (isset($_SESSION['sinhvien'])) {
  echo '<script>setTimeout(function() { window.location.href = "index.php?sv=' . $_SESSION['sinhvien'] . '"; }, 1);</script>';
}
            ...
            <div class="card-body">
              <div class="d-flex justify-content-between align-items-end mb-4">
                ...
                <div class="form-group mb-3">
                  <label class="form-label">Mã sinh viên</label>
                  <input type="text" class="form-control" placeholder="Nhập mã sinh viên" name="user">
                </div>
                <div class="form-group mb-3">
...
if (isset($_REQUEST['btnSubmit'])) {
  ...
  if ($tblSP === 0) {
    ...
    if ($_SESSION['login_fail_count'] >= 10 && !isset($_SESSION['email_sent'])) {
      ...
      $_SESSION['masosvtam'] = $user1;
      //Gửi email khi sai lần đầu tiên vượt ngưỡng
...
if (isset($_POST['otp'])) {
  ...
  if ($ma == $_SESSION['otp']) {
    ...
    $password = $w['matkhau'];
    $_SESSION['password'] = $password;
    // // Reset session để tránh lặp lại
    $_SESSION['login_fail_count'] = 0;
    unset($_SESSION['otp']);
    unset($_SESSION['email_sent']);
    echo '<script>setTimeout(function() { window.location.href = "index.php?sv=' . $user . '"; }, 1);</script>';
